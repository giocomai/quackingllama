...
By default, the database is stored in the current working directory, but
...
For example, if we ask to describe the logo of this package, we get the
following reponse:
...
prompts should be structured. The function
[`ql_translate()`](https://giocomai.github.io/quackingllama/reference/ql_translate.md)
streamlines the process: it is enough to provide an input text, and the
target language, and all the rest is managed internally. Input language
